...
- `dynamic_cast`**运算符**：在继承体系中进行安全地多态向下转型(基类->派生类)或跨转型
...
2. [[variant | std::variant(类型安全联合体)]] + `std::visit` (C++ 17)：用于有限集合的静态多态，在编译期确定访问逻辑，性能更改且类型安全
...
---
### 总结
RTTI 是 C++ 提供的一项"安全网"技术，在必要时可能解燃眉之急，但不应成为日常设计的依赖。**好的面向对象设计应尽量让虚函数承担职责**，将RTTI限制在架构底层(如 工厂、反射机制) 或辅助调试中。如果发现自己频繁书写 `dynamic_cast`, 往往是设计需要重构的信号。